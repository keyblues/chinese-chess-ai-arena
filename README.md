...
- 默认上下文窗口 **256k**（262144）、最大输出 **64k**（65536）。双方各自保留**完整**跨回合会话（盘面、tool 往返、`reasoning_content` 均不截断）；占用达到窗口的 **80%**（且不超过「窗口 − 输出上限」）时，由同一模型把更早回合**摘要压缩**成一条记忆（失败则整回合丢弃），最近若干回合保持原文。完整会话存 IndexedDB，棋谱等轻量状态仍在 localStorage。
